...
void MapRenderer::add_buses_name(std::vector<std::pair<Bus*, int>>& buses_palette){    
    ...
    for (auto [bus, palette] : buses_palette) {  
        ...
        if (!bus_empty) {
            ...
            if (bus->is_roundtrip) {
                set_route_text_additional_properties(route_name_roundtrip,std::string(bus->name),sphere_projector(stops_geo_coords[0]));
                map_svg.add(route_name_roundtrip);
                
                set_route_text_color_properties(route_title_roundtrip, std::string(bus->name), palette, sphere_projector(stops_geo_coords[0]));
                ...
                if (stops_geo_coords[0] != stops_geo_coords[stops_geo_coords.size()/2]) {
                    set_route_text_additional_properties(route_name_notroundtrip, std::string(bus->name), sphere_projector(stops_geo_coords[stops_geo_coords.size()/2]));
                    map_svg.add(route_name_notroundtrip);
                    
                    set_route_text_color_properties(route_title_notroundtrip, std::string(bus->name), palette, sphere_projector(stops_geo_coords[stops_geo_coords.size()/2]));
